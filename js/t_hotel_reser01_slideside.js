...
$(".shortReservation_wrap_up").on("submit",function(e){ 
    ...
    for( key in priceExp) {
        const q = priceExp[key];
        $(`.${key}`).find(".cs_e_title").text(q[0]);
        $(`.${key}`).find(".cs_e_pre").text(q[1]);
        $(`.${key}`).find(".cs_e_notice").text(q[2]);
    }
    // 1. 기본 객실 로드 //
    ...
    $(".ri_e_t_choice_extra").each(function(){
        ...
        $(this).find("li").each(function(){
            ...
            w[0].forEach((v,i) => {
                v += "<br/>";
               const t = $(this).find(".cs_e_title").text();
               $(this).find(".cs_e_title").text( t + v );
            })
            $(this).find(".cs_e_pre").text(w[1].map(v => v + `<br/>`));
            $(this).find(".cs_e_notice").text(w[2].map(v => v + `<br/>`)); 
        })
